...
int main(){
    ...
    while(peekChar() != EOF){
        ...
        // if c is a newline char or a space,
        //   checks if the current word being built is empty.
        //      if it isn't, searches for it in l's linked list.
        //          if it is found, replaces it in the output with its index.
        //          if it is not found, puts it in the output list
        //              either way, we reset the current word.
        //      if the word is empty, adds the space/newline to the output and moves on
        // if c is not a newline or space, adds c to the current word being built
        if(c == '\n'){
